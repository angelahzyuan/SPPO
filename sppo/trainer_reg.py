...
class SPPOTrainer(Trainer):
    ...
    def sppo_loss(
        ...
        # For sppo
        logits_w = policy_chosen_logps - policy_chosen_logps.detach() # - reference_chosen_logps
        logits_l = policy_rejected_logps - policy_rejected_logps.detach() # - reference_rejected_logps
        logits_diff_w = reference_chosen_logps - policy_chosen_logps
        logits_diff_l = reference_rejected_logps - policy_rejected_logps
        ...
        if self.loss_type == "sppo":
            ...
            loss_w = (logits_w - (1 / self.beta)*(chosen_probs_win - 0.5)) ** 2
            loss_l = (logits_l - (1 / self.beta)*(chosen_probs_lose - 0.5)) ** 2
            reg = (logits_diff_w).exp().mean() + (logits_diff_l).exp().mean()
            losses = (loss_w + loss_l)/2 + self.reg_coef * reg
        elif self.loss_type == "sppo_reversekl":
            loss_w = (logits_w - (1 / self.beta)*(chosen_probs_win - 0.5)) ** 2
            loss_l = (logits_l - (1 / self.beta)*(chosen_probs_lose - 0.5)) ** 2
            reg = (logits_diff_w ** 2).mean() + (logits_diff_l ** 2).mean()
            losses = (loss_w + loss_l)/2 + self.reg_coef * reg
